...
{
    public class PaymentBiz : _Service
    {

        public async Task<Json> InsertLog(RequestCP model)
        {
            Json result;
            try
            {
        ...
        public async Task<ServiceResultClient> PaymentRequest(RequestPay model)
        {
            ...
            {
                ServiceResultClient serviceResult;
                RequestCP requestCP = new RequestCP();
                ...
                requestCP.merchantID = reqCP.merchantID = Constants.merchantID;
                requestCP.terminalID = reqCP.terminalID = Constants.terminalID;
                requestCP.amount = reqCP.amount = model.amount.ToString();
                requestCP.date = reqCP.date = Constants.date;
                requestCP.time = reqCP.time = Constants.time;
                requestCP.localIP = reqCP.localIP = Constants.LocalIP;
                requestCP.PAN = reqCP.PAN = model.PAN;
                reqCP.PINBlock = model.PINBlock;
                requestCP.serial = reqCP.serial = Constants.serial;
                requestCP.STAN = reqCP.STAN = Constants.STAN;
                reqCP.track2Ciphered = model.track2Ciphered;
                reqCP.custMobile = Constants.mobile;
                requestCP.invoiceNo = reqCP.invoiceNo = Constants.invoiceNo;


                FanAvaServices_Mobile.SoapMobileClient s = new FanAvaServices_Mobile.SoapMobileClient();


                var resultRequestCP = s.requestCP(reqCP);
                ...
                {
                    requestCP.Success = false;
                    var resultLog = await InsertLog(requestCP);
                    requestCP.refNo = resultRequestCP.refNo;
                    return resultRequestCPMessage;
                }
                else
                {
                    var settleReverseIn = new FanAvaServices_Mobile.settleReverseIn()
                    {
                        authInfo = reqCP.authInfo,
                        custMobile = reqCP.custMobile,
                        date = reqCP.date,
                        localIP = reqCP.localIP,
                        merchantID = reqCP.merchantID,
                        orgRefNo = resultRequestCP.refNo,
                        terminalID = reqCP.terminalID,
                        serial = reqCP.serial,
                        time = reqCP.time,
                        orgAmount = reqCP.amount,
                        transType = "buy",
                        STAN = reqCP.STAN,
                        version = reqCP.version
                    };
                    var resultSettlement = s.settlement(settleReverseIn);
                    var resultSettlementMessage = TransactionMessage(resultSettlement.responseCode);
                    ...
                    requestCP.Success = resultSettlementMessage.status == StatusTransaction.SuccessfulTransaction.ToString() ? true : false;
                    var resultLog = await InsertLog(requestCP);
